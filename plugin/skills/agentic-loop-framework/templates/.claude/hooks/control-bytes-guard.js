...
// or 0x7F). Born from a recurring failure class (workflow-retro optimizer): a literal
...
// mirrors json-guard.js / dashboard-guard.js (fail-open on our own errors).
...
process.stdin.on('end', () => {
  ...
  console.error(
    ...
    + 'Rebuild the affected text via a node -e script with fs.writeFileSync (never hand-type/edit '
    + 'the escape), then re-check.'
  );
  process.exit(2);
});
