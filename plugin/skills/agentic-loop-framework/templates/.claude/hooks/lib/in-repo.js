...
// Guarded-repo containment (pure helper) — see the agentic-loop-framework CHANGELOG.
//
// Every Edit|Write guard decides WHAT to enforce from the file path alone, so an
// absolute path in ANOTHER repository that happens to carry a guarded segment (lib/,
// drivers/, docs/dashboard/, a manifest basename) was treated as one of the guarded
// project's own source files. An adopting project hit exactly that: a session edited a
// sibling checkout and was blocked over a documentation convention that repo does not
// use. Guards enforce THEIR project's conventions, so they must stay inside THAT
// project's tree.
//
// The bound is the REPOSITORY containing the session cwd, not the cwd itself: a session
// started in a subdirectory (cwd = <repo>/sub/dir) must still guard <repo>/lib/*, and
// the hook input's cwd is the session dir, which may be a subdir or parent of the edited
// file's repo. Both sides are canonicalised before comparing, because the same location
// can be spelled differently on Windows (8.3 short names, junctions) and a raw string
// compare would silently skip a real violation.
