...
// SessionStart hook — makes a disarmed gate net AUDIBLE at session start. The commit-time
// invariant (test/hooks/hook-command-paths.test.js) only fires when a commit happens; the
// window before that — a fresh worktree without `npm ci`, a stale second clone, an
// unresolvable hook path — is where the origin project produced gate-skip records nobody
// read, and 211 hook launches that failed silently over five days because a relative
// script path did not resolve from the session cwd.
//
...
// is the common case). "A fail-open path must be audible" is the framework rule this hook
// generalizes to session start.
