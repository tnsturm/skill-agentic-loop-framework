...
//
// Why it exists: the commit-time invariant (hook-command-paths.test.js) only fires when a
// commit happens. The window before that — a fresh worktree without `npm ci`, a stale
// second clone, an unresolvable hook path — is exactly where the origin project produced
// gate-skip records that nobody read. A SessionStart hook prints its findings to stdout,
// which Claude Code injects into the session context, so the MODEL sees the warning — not
// just a ledger file.
//
