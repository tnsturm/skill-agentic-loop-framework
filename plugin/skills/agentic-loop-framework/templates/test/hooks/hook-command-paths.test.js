...
// green".
//
// This is the same silent-degradation class the framework already guards elsewhere (a gate
// that fails open must be audible), one step worse: the process never starts, so not even
// the hook ledger records it. It was found in the origin project by scanning transcripts,
// not by anyone noticing — 211 such failures over five days, from four different session
// cwds: a sibling repo, .claude/worktrees, .claude/hooks itself, and a second checkout's
// .git/worktrees.
//
// ${CLAUDE_PROJECT_DIR} is substituted by Claude Code itself before the command reaches a
// shell, so it is shell-independent. The bare $CLAUDE_PROJECT_DIR form is NOT: hooks that
// match Bash|PowerShell also run under PowerShell, which reads $CLAUDE_PROJECT_DIR as an
// undefined variable ($null) — the CLI emits a warning for exactly that spelling.
...
// --- the repro ---------------------------------------------------------------
// Both halves run a hook script the way Claude Code would, from a cwd that is NOT the repo
// root. The negative control pins the defect; the positive one pins the fix. Pick any hook
// the template ships — test-gate is present in every generated project.

const PROBE = 'test-gate.js';
