...
- **`/documenting-code`** (optional project skill — create it once the project keeps specs under `docs/superpowers/`): apply to source files with spec-relevant logic — spec-referenced file header, decision-point comments with §-refs, JSDoc on pure library exports. Pure mechanics (rename, format fix) only need the existing header kept accurate. Until the skill exists, follow the same rules manually.
...
The `milestone-checkpoint` skill enforces this at update time (its step 4).
...
scripts) stay here. The milestone-checkpoint checks for drift (step 7a).