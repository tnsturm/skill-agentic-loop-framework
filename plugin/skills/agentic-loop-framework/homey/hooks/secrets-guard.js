...
// out-of-band — the exact device write password.
//
// Credentials must live ONLY in the device store and be read at runtime. This
// file is itself tracked, so it deliberately embeds NO secret: the exact
// password is supplied (optionally) via the DEVICE_WRITE_PASSWORD env var, and
// no error message ever echoes a matched value.
//
// Pattern mirrors check-version-sync.js (fail-open on our own errors).
...
// error text even printed the path without its `test/` prefix. Deliberate residual
// scope: a real credential pasted into a test file is NOT caught here — review (§9) is
// the net for that, and SR-01/SR-02 govern the shipped app, not the test harness.
...
function violation(content, knownSecret) {
  ...
  // C. The exact device write password, if the operator wired it in out-of-band
...
process.stdin.on('end', () => {
  ...
  const knownSecret = (process.env.DEVICE_WRITE_PASSWORD || '').trim() || null;
  ...
  if (rule) {
    ...
    console.error(
      `secrets-guard: refusing to write what looks like a hardcoded device write `
      + `credential (rule: ${rule}) into ${rel}. Credentials must live ONLY in the `
      + `device store and be read at runtime. If this is a false positive, adjust `
      + `the value to read from store/settings instead of a literal.`
    );
