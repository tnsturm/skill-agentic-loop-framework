...
// Smoke test for homey/hooks/json-guard.js (PostToolUse Edit|Write, install into a
// project's .claude/hooks/) — blocks invalid manifest/changelog JSON (exit 2) and
// leaves valid / non-guarded / non-JSON files alone (exit 0). Ported from VioletApp's
// test/hooks/json-guard.test.js.
...
const HOOK = path.join(__dirname, '..', '..', 'hooks', 'json-guard.js');
...
test('json-guard: absolute path outside cwd (another repository) → exit 0', () => {
  // Every repo on disk has a package.json; only this project's is ours to gate.
