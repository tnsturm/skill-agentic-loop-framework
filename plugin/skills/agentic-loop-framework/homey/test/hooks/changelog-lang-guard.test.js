...
// Smoke test for homey/hooks/changelog-lang-guard.js (PostToolUse Edit|Write, install
// into a project's .claude/hooks/) — blocks (exit 2) an edit to .homeychangelog.json that
// leaves the CURRENT .homeycompose/app.json version without a complete en+de entry;
// passes through otherwise. Uses the shared lib/changelog.js completeness check — pair
// this with a release-gate-equivalent hook if/when one exists in this module, so the
// same rule is enforced both at edit-time and at release-command time.
...
const HOOK = path.join(__dirname, '..', '..', 'hooks', 'changelog-lang-guard.js');
const VERSION = '0.9.9';
