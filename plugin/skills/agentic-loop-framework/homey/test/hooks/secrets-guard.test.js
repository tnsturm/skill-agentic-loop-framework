...
// Smoke test for homey/hooks/secrets-guard.js (PreToolUse Edit|Write, install into a
// project's .claude/hooks/) — blocks a hardcoded Basic auth token or a
// writePassword/writeUsername string literal (exit 2) in tracked app files, and passes
// clean code / store reads / non-guarded paths (exit 0). Its error text must never echo
// the matched secret. Ported from VioletApp's test/hooks/secrets-guard.test.js — the
// generic hook reads its optional known-secret from DEVICE_WRITE_PASSWORD (not
// VIOLET_WRITE_PASSWORD).
...
const HOOK = path.join(__dirname, '..', '..', 'hooks', 'secrets-guard.js');
...
const cases = [
  ...
  // 2026-08-30: the lib//drivers/ rules matched ANYWHERE in the path, so test/drivers/
  // and test/lib/ were guarded too — against this hook's own header and against the
  // *.json rule, which already excluded test/. A fixture credential in a test file is
  // not a leak; the false alarm cost a detour via a constant during a fix round.
