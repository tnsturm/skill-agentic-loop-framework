...
// Smoke test for homey/hooks/release-gate.js (PreToolUse Bash, install into a project's
// .claude/hooks/) — the gate blocks `homey app install|publish` (exit 2) when (a)
// .homeychangelog.json lacks a complete en+de entry for the compose version, (b) the
// version is already logged in docs/dashboard/versions.md (forgotten bump = double
// release), or (c) publish AND a docs/superpowers/security/credential-rotation.md
// already exists but has no dated rotation proof. Fails open (exit 0) on non-release
// commands, non-compose repos, malformed stdin. Ported from VioletApp's
// test/hooks/release-gate.test.js, adjusted for the one intentional drift: this generic
// hook's part (c) is SOFT — a MISSING credential-rotation.md means the app has no
// device credential to rotate and publish is allowed (VioletApp's original always
// requires the file, since it always manages a device credential).
...
const HOOK = path.join(__dirname, '..', '..', 'hooks', 'release-gate.js');
const VERSION = '0.9.9';
...
test('release-gate: version only mentioned in prose (planned next upload) → PASS', () => {
  ...
  // version is not a logged release and must not block (table rows only).
...
test('release-gate: publish, no credential-rotation.md at all → PASS (no device credential to rotate)', () => {
  const { code, err } = runHook('homey app publish', makeRepo({ changelog: FULL_CHANGELOG }));
  assert.strictEqual(code, 0, err);
});

test('release-gate: publish, credential-rotation.md exists but undated → BLOCK naming the rotation', () => {
...
test('release-gate: install never requires rotation proof, even with an undated rotation file → PASS', () => {
  const { code, err } = runHook('homey app install', makeRepo({ changelog: FULL_CHANGELOG, rotation: 'todo' }));
  assert.strictEqual(code, 0, err);
});

test('release-gate: multiple violations are all reported', () => {
  const { code, err } = runHook('homey app publish', makeRepo({ logged: true, rotation: 'todo' }));
  assert.strictEqual(code, 2, err);
  assert.match(err, /\.homeychangelog\.json/);
  assert.match(err, /versions\.md/);
  assert.match(err, /credential-rotation\.md/);
