...
// Smoke test for homey/hooks/compose-guard.js (PreToolUse Edit|Write, install into a
// project's .claude/hooks/) — blocks a direct edit/write of the GENERATED root app.json
// (exit 2) in a Homey Compose project, and passes edits to the compose sources, other
// files, and non-compose repos where app.json is the real source (exit 0). Fails open
// (exit 0) on its own errors. Ported from VioletApp's test/hooks/compose-guard.test.js.
...
const HOOK = path.join(__dirname, '..', '..', 'hooks', 'compose-guard.js');
...
// and the guard must not block it.
...
test('compose-guard: edit lib source → PASS', () => {
  const dir = makeProject(true);
  const { code } = runHook({ file_path: 'lib/DeviceClient.js', new_string: 'x' }, dir);
  assert.strictEqual(code, 0);
});
